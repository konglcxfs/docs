...
| `kling/` | Kling (video) | `kling/kling-v3-omni`, `kling/kling-v2-6` â€?see [Kling Video](/other-apis/kling-video) |
...
default listing in `/v1/models` always uses the prefixed form â€?code
...
| Web search built-in | `openai/gpt-4o-search-preview` (chat completions), or any Grok model on `/v1/responses` with the `web_search` tool â€?see [Web search](/advanced/web-search) |
...
| Auto-pick cheapest | `clearmaas/auto` â€?see [Auto Router](/routing/auto-router) |
| Video generation | `kling/kling-v3-omni`, `kling/kling-v3` â€?see [Kling Video](/other-apis/kling-video) |
