...
3. `OPENCODE_CONFIG` env var â€?custom overrides
4. Project config (`opencode.json` in project root) â€?project-specific
5. `OPENCODE_CONFIG_CONTENT` env var â€?runtime overrides
...
- `timeout` â€?Request timeout in milliseconds (default: 300000)
- `setCacheKey` â€?Always set a cache key for designated provider
- `baseURL` â€?Override the default API endpoint
