...
The Anthropic SDK that Claude Code uses appends `/v1/messages` itself, so the base URL must be the **bare host â€?no `/v1` suffix**.
...
- `.claude/settings.json` â€?project-scoped
- `~/.claude/settings.json` â€?user-scoped
